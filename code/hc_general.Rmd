...
```{r}
path = "x_疾病类型：Kidney Chromophobe (116)"
lines <- readLines("排序(列名).txt")
dir.create(path,showWarnings = FALSE, recursive = FALSE)
data = read.csv(paste(path, ".csv", sep=""), row.names = 1)
data <- data[, lines]

col_fun = colorRamp2(c(0,0.077,0.078,0.079,
                       0.081,0.15,0.3,0.5,1), 
                     c("white","white","#99CCFF","#3399FF", "#0080FF","#0000CC", "#000066", "#000033","black"))
...
color = (df[1,] - min(df[1, ])) / (max(df[1, ])- min(df[1, ]))
color = as.matrix(color)
start = (df[1,]) / sum(df[1,]) * (32^4)
end = cumsum(start)
png("KICH.png")
hc = HilbertCurve(0, 32^4, start_from = "topleft", first_seg = "horizontal", title = "KICH",mode = "pixel", level = 9)
hc_layer(hc, x1 = start, x2 = end, col =col_fun(color))
hc_polygon(hc, x1 = start, x2 = end)
dev.off()
```


```{r}
path = "x_疾病类型：Adrenocortical Carcinoma (79)"
lines <- readLines("排序(列名).txt")
dir.create(path,showWarnings = FALSE, recursive = FALSE)
data = read.csv(paste(path, ".csv", sep=""), row.names = 1)
data <- data[, lines]
